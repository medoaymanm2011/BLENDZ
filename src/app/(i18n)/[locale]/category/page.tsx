...
import { listCategories } from '@/server/controllers/categoriesController';

// Force dynamic rendering for this route
export const dynamic = 'force-dynamic';
export const revalidate = 0;

type DbCategory = { _id: string; name: string; slug: string; image?: string | null };

export default async function CategoriesIndexPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale });

  // Query DB directly (no HTTP fetch)
  await connectToDB();
  const categoriesData = (await listCategories()) as unknown as DbCategory[];
  ...
  const Card = ({ c, className, imgClass }: { c: DbCategory; className?: string; imgClass?: string }) => (
    <Link
      href={`/${locale}/search?categories[0]=${encodeURIComponent(c._id)}`}
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl md:text-3xl font-extrabold text-gray-800">{t('filters.categories')}</h1>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-5">
          {rest.map((c) => (
            <Card key={c._id} c={c} className="h-44 md:h-52 lg:h-56" />
          ))}
        </div>
      </main>
      <Footer />
